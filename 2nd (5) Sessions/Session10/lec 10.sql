...
lw for update yb2a h3ml feha 7aga, y3ny h3dl 7aga fel data elle fel 
...
---------------------------------------------------
--backup, store
-- adek 4oft bt3ml backup azay wizzard method y3ny btdos 3la l db tasks backup w a5tar l anwa3 bta3ty ml 3 w m2dr4 a5ter diffnitional m 8er full
--wlma age arg3ha b2a hdos 3la l db restore from device w aselect l file w a22dr a7dd timeline fl transaction log 
--dlw2ty 
backup database ITI
to disk = 'D:\my backup db\iti.bak'
--W L tr2a l talta ane a3ml jop jop deh 7aga bttexecute b time mo3yn bs f a7ot feha l query ele fo2 dh w a7ot l time bta3e
------------------------------------------
--reset identity
...
delete from T1  where Column_3 between 2 and 20 --hyntog 7aga asmha idenitiy gap an f m arkam m 3 l 19 m4 m7tot feha data w mt2dr4 t7ot fehom 7aga 34an dh identity byincrease auto
-- f bn3ml on w off llcolumn dh w a22dr ane a7ot fl arkam deh 3ade
...
truncate table t1
-- diff between truncate w delete an truncate b treset l idenitiy f a22dr m3 awl value gdeda htt7t htla2eha wa7d awl idenitity 3ade '' truncate m4 byge m3aha where
GO
...
select @@identity --byt2sr b a5r query mtexecute 

SELECT IDENT_CURRENT('t1') --byegp a5r ident fl table dh
--------------------------------------------
--insert statments 
--simple insert
--insert constructor
...
--ama age a3mlha restore b2a
restore database iti 
from database_snapshot='itisnap' -- asm l ss bta3k 
-- mofeda lw na hrestore 3la nfs l server y3ny a7sn ml backup 

--diff between ss and backup 
--mlha4 mdf wla ldf 
--a22dr arg3 b time fl backup w log w data
--l backup m4 lazm nfs l sever 
--read only db w feha pointers 3la ldata 
------------------------------------------------
--SQL CLR 
